...
void task_init(void) {
	...
	current = (struct task_struct *)0xffffffe000ff0000;
	task[0] = current;
	...
	task[0]->blocked = 0;
	task[0]->pid = 0;
	(task[0]->thread).sp = (unsigned long long)(0xffffffe000ff0000 + TASK_SIZE);
	// initialize task[1-4]
	int i;
	for(i = 1; i <= 4; i++) {
		task[i] = (struct task_struct *)(0xffffffe000ff0000 + TASK_SIZE * i);
		task[i]->state = TASK_RUNNING;
		task[i]->blocked = 0;
		task[i]->pid = i;
		(task[i]->thread).sp = (unsigned long long)(0xffffffe000ff0000 + TASK_SIZE * (i+1));
		// initialize ra
...
void schedule(void) {
	...
	// switch
	puts("[!] Switch from task "); puti(current->pid);
	puts(" [task struct: "); putlonglong((unsigned long long)current);
	puts(", sp: "); putlonglong((current->thread).sp); puts("]");
	puts(" to task "); puti(task[next_task]->pid); 
	puts(" [task struct: "); putlonglong((unsigned long long)task[next_task]);
	puts(", sp: "); putlonglong((task[next_task]->thread).sp); puts("]");
	puts(", prio: "); puti(task[next_task]->priority);
	puts(", counter: "); puti(task[next_task]->counter); puts("\n");
	// reset priority
